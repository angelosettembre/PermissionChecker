...
public class TCPOutput implements Runnable
{
    ...
    private LocalVpnServiceTest vpnService;
    ...
    public TCPOutput(ConcurrentLinkedQueue<Packet> inputQueue, ConcurrentLinkedQueue<ByteBuffer> outputQueue,
                     Selector selector, LocalVpnServiceTest vpnService)
    {
    ...
    private void initializeConnection(String ipAndPort, InetAddress destinationAddress, int destinationPort,
                                      Packet currentPacket, Packet.TCPHeader tcpHeader, ByteBuffer responseBuffer)
            throws IOException
    {
        ...
        if (tcpHeader.isSYN())
        {
            ...
            try
            {
                ...
                if (outputChannel.finishConnect())
                {
                    ...
                    currentPacket.updateTCPBuffer(responseBuffer, (byte) (Packet.TCPHeader.SYN | Packet.TCPHeader.ACK),
            ...
            catch (IOException e)
            {
                ...
                currentPacket.updateTCPBuffer(responseBuffer, (byte) Packet.TCPHeader.RST, 0, tcb.myAcknowledgementNum, 0);
                TCB.closeTCB(tcb);
            }
        }
        else
        {
            currentPacket.updateTCPBuffer(responseBuffer, (byte) Packet.TCPHeader.RST,
    ...
    private void processFIN(TCB tcb, Packet.TCPHeader tcpHeader, ByteBuffer responseBuffer)
    {
        synchronized (tcb)
        {
